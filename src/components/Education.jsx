...
const eduData = [
  {
    ...
    details: "Focusing on Software Architecture, Algorithms, and Full Stack Development."
  },
  {
  ...
  return (
    <section id="education" className="py-12 sm:py-16 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-[1.5rem] sm:text-4xl md:text-5xl font-bold">Academic Background</h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          {eduData.map((edu, index) => (
            ...
            >
              ...
              <h3 className="text-base sm:text-lg font-bold mb-2 leading-snug">{edu.degree}</h3>
              <p className="text-blue-500 text-xs sm:text-sm font-semibold mb-2 sm:mb-3">{edu.school}</p>
              <p className="text-gray-400 text-[10px] sm:text-xs uppercase tracking-widest mb-3 sm:mb-4 font-medium">{edu.session}</p>
              <div className="h-px bg-gray-200 dark:bg-white/5 mb-3 sm:mb-4" />
              <p className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm leading-relaxed">{edu.details}</p>
            </motion.div>
          ))}
        </div>
      </div>
